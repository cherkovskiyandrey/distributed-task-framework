...
public class LocalAtLeastOnceWorker implements TaskWorker {
    ...
    @Override
    public <T, U> void execute(TaskEntity taskEntity, RegisteredTask<T> registeredTask) {
        ...
        try {
            ...
            var stateHolder = workerContextManager.getCurrentContext()
            ...
            try {
                ...
                if (isStatefulTask) {
                    ...
                    hasToInterruptRetrying = statefulTaskOpt.orElseThrow()
                        .onFailureWithResult(failedExecutionContext, (StateHolder<U>) stateHolder);
                } else {
            ...
            var localStateSerialized = serializeMessageSilently(taskId,
                stateHolder.get().orElse(null),
                finalTaskEntity
            ).orElse(null);
            ...
            try {
                taskEntity = taskEntity.toBuilder()
                    ...
                    .localState(localStateSerialized)
                    .build();
    ...
    private Optional<byte[]> serializeMessageSilently(TaskId taskId,
        ...
        try {
            return Optional.ofNullable(taskSerializer.writeValue(message));
        } catch (IOException e) {
            getEngineErrorCounter(finalTaskEntity).increment();
            log.warn("serializeMessageSilently(): can't write local state for taskId=[{}]", taskId, e);
            return Optional.empty();
        }
    }
    ...
    protected <T, U> void runInternal(final TaskEntity taskEntity, RegisteredTask<T> registeredTask) {
        ...
        if (taskEntity.isCanceled()) {
            ...
            U localState = isStatefulTask && taskEntity.getLocalState() != null ?
                taskSerializer.readValue(taskEntity.getLocalState(), statefulTaskOpt.get().stateDef().getJavaType()) :
                null;
            var stateHolder = workerContextManager.getCurrentContext()
                .orElseThrow(() -> new IllegalStateException("Context hasn't been set"))
                .getStateHolder();
